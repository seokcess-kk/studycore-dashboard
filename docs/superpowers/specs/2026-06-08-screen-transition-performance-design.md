...
- **작업 중 백그라운드 갱신 깜빡임**: 백그라운드 `loadAll` 결과가 캐시와 **동일하면 재렌더·캐시 저장을 생략**한다(직렬화 문자열 비교). 대부분의 재방문(데이터 무변경)에서 화면을 건드리지 않아, 관리자가 검색 입력 등으로 조작 중일 때 목록이 다시 그려지는 깜빡임을 막는다. 데이터가 실제로 바뀐 경우에만 갱신한다.
